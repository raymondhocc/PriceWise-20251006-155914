...
export type ToolResult = WeatherResult | {
  content: string;
} | ErrorResult;
...
const extractTextFromHtml = (html: string): string => html.replace(/<(script|style|noscript)[^>]*>[\s\S]*?<\/\1>/gi, '').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
async function fetchWebContent(url: string): Promise < string > {
  try {
    ...
    return text.length ? `Content from ${url}:\n\n${text.slice(0, 8000)}${text.length > 8000 ? '...' : ''}` : `No readable content found at ${url}`;
  } catch (error) {
